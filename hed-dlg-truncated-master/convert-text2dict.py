...
if args.dict != "":
    ...
    if args.cutoff != -1:
        ...
        vocab_count = word_counter.most_common(args.cutoff) #list 中包着tuple,(token,出现的次数)
    else:
    ...
    for (word, count) in vocab_count:
        if not word in vocab:
            vocab[word] = i
            i += 1
        #把（token，出现的次数）换成 （token：token_id）
...
# document freq某一个单词A，在多少个dialogue中出现过
...
for line, dialogue in enumerate(open(args.input, 'r')):
    ...
    for word in dialogue_words:
        ...
        freqs[word_id] += 1 #term_freq,某一个单词A，出现过多少次
    ...
    for word_id in unique_word_indices:
        df[word_id] += 1 #这些去重后的单词在当下这个dialogue中出现了一次
