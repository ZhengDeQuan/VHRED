        ...
        while not diter.exit_flag:
            ...
            while len(dialogues) < diter.batch_size:
                ...
                if diter.max_len == -1 or len(s) <= diter.max_len:
                    dialogues.append([s]) #s一个list，里面装的是一个dialogue中的所有的token_id ['我的','名字'，'叫'，'郑权'，'</s>','你'，'叫','什么']
                                          #所以[s],是一个list中再套一个list
...
class SSIterator(object):
    ...
    def load_files(self):
        ...
        self.data_len = len(self.data)#全部数据的总长度
    ...
    def start(self):
        ...
        self.gather.start()# SSFetcher类继承了threading.Thread类，估计是threading.Thread里面有这个start()方法,用来开启多线程用的吧
